...
public static void placeOrder(Retail esql) {
	try{
		...
		while(a){
			...
			if(esql.sID.contains(Integer.parseInt(id)) || esql.authorisedUser.get(5).replaceAll("\\s", "").equals("admin")){ 
				a = false;
			}else{
...
public static void viewRecentOrders(Retail esql) {
    try{
		if (esql.authorisedUser.get(5).replaceAll("\\s", "").equals("customer") ||esql.authorisedUser.get(5).replaceAll("\\s", "").equals("admin")){
